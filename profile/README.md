...
<p align="center">Free file-type icon sets as embedded SVG for pure-Go UIs.</p>
<p align="center">
...
Toolkits that render their own UI need file-type icons, and the good free sets
ship as loose SVG files with no Go packaging. **go-icons** closes that gap: each
repo embeds a curated subset of one free, MIT-licensed icon set and exposes the
same tiny contract — `Icon(filename)` returns the SVG document for a file
(matched by exact base name, then by extension, then a generic-document
fallback) and `Folder()` returns the folder glyph.
